...
export const fetch_data_underFilters = createAsyncThunk(
  ...
  async (first_params, second_params, third_params, forth_params) => {
    const response = await axios.get(
      `http://localhost:7001/api/coolcicks/v1/trendsUnderCategory?category=${first_params}&color=${second_params}&price${third_params}&company=${forth_params}`
    );
    return response.data;
  }
);
...
export const fetchData_underFeatures = createAsyncThunk(
  ...
  async () => {
    const res = await axios.get(
      "http://localhost:7001/api/coolcicks/v1/featured"
    );
    return res.data;
  }
);
